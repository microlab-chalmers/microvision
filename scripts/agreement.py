...
from microvision.config import AREA_THSD, IMG_HEIGHT, IMG_WIDTH, OBJECT_CLASSES
...
def compute_pairwise_agreement(
    annotator1_path, annotator2_path, img_width, img_height, area_ranges
):
    """
    ...
    for ann1_txt_file in ann1_files:
        ...
        if num_ann1 > 0 and num_ann2 > 0:
            ...
            matched_ann1 = set(
                r for r, c in zip(row_ind, col_ind) if cost_matrix[r, c] < 1.0
            )
            matched_ann2 = set(
                c for r, c in zip(row_ind, col_ind) if cost_matrix[r, c] < 1.0
            )
...
def print_agreement_summary(all_kappas, all_ious, categories_dict):
    ...
    header += (
        "small_kappa".ljust(15)
        + "medium_kappa".ljust(15)
        + "large_kappa".ljust(15)
        + "ALL_kappa".ljust(15)
    )
    header += (
        "small_IoU".ljust(12)
...
if __name__ == "__main__":
    ...
    # --- 2. Define your paths ---
    BASE_DIR = "./data/agreement"

    # --- 3. Define categories ---
    CATEGORIES = [
        {"id": k, "name": v, "supercategory": "none"} for k, v in OBJECT_CLASSES.items()
    ]
    CATEGORIES_DICT = {c["id"]: c["name"] for c in CATEGORIES}

    # --- 4. Define your custom area ranges (in pixels squared) ---
    s_max = AREA_THSD["small"]
    m_max = AREA_THSD["medium"]
    l_min = m_max
    ...
    # --- 5. Compute all pairwise agreements ---
    ...
    for ann1, ann2 in pairs:
        ...
        results = compute_pairwise_agreement(
            ann1_path, ann2_path, IMG_WIDTH, IMG_HEIGHT, AREA_RANGES
        )
        pairwise_results[pair_name] = results
        print(f"  Computed {pair_name}")

    # --- 6. Calculate averages across the three pairs, by class and overall ---
    avg_kappas = {
    ...
    for size_cat in ["all", "small", "medium", "large"]:
        ...
        kappas = [
            pairwise_results[pair][size_cat]["kappa"] for pair in pairwise_results
        ]
        ...
        for class_id in CATEGORIES_DICT.keys():
            ...
            for pair in pairwise_results:
                if class_id in pairwise_results[pair]["by_class"]:
                    class_kappas.append(
                        pairwise_results[pair]["by_class"][class_id][size_cat]["kappa"]
                    )
                    class_ious.append(
                        pairwise_results[pair]["by_class"][class_id][size_cat]["iou"]
                    )

            if class_kappas:
                avg_kappas[class_id][size_cat] = {"average": np.mean(class_kappas)}
                avg_ious[class_id][size_cat] = {"average": np.mean(class_ious)}
            else:
